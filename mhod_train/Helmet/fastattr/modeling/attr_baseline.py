...
from .bce_loss import cross_entropy_sigmoid_loss,corss_entropy_softmax_loss
...
class AttrBaseline(Baseline):
    ...
    def losses(self, outputs, gt_labels):
        ...
        if "SoftmaxCrossEntropyLoss" in loss_names:
            bce_kwargs = self.loss_kwargs.get('bce')
            loss_dict["loss_bce"] = corss_entropy_softmax_loss(
                cls_outputs,
                gt_labels,
                self.sample_weights,
            ) * bce_kwargs.get('scale')
