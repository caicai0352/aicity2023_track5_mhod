...
class AttrEvaluator(DatasetEvaluator):
    ...
    def get_attr_metrics(gt_labels, pred_logits, thres):
        ...
        np.savetxt('/mnt/home/aicity_track5/crop_images/0308_pre.txt',index_list)
         ...
         #打印预测分数，预测类别
        # AttrEvaluator.json_result(index_list, label_class_map,pred_logits)
        # print(f"哈哈{index_list}")
        ...
        np.savetxt('/mnt/home/person_att/fast-reid-master/projects/DrvPassger/prc/precision.txt',precision)
        np.savetxt('/mnt/home/person_att/fast-reid-master/projects/DrvPassger/prc/recall.txt',term1)
        np.savetxt('/mnt/home/person_att/fast-reid-master/projects/DrvPassger/prc/ma.txt',label_mA_verbose)
    ...
    def json_result(index_list, label_class_map,pred_logits):
        test_nums=index_list.shape[0]
        index_list=index_list.astype(np.int16)
        imgs_paths=[]
        with open('/mnt/home/person_att/file_names.txt', "r", encoding='utf-8') as f:
            for line in f.readlines():                          #依次读取每行  
                line = line.strip()
                # print(line)
                imgs_paths.append(line) 

        # print(f"hhhhhhhh{imgs_paths}")
        for i  in range(test_nums):
            attr = OrderedDict()
            index_begin = 0
            j=0
            for key in label_class_map:
                # print(j,key)
                index=index_begin + index_list[i][j]
                attr[key]=OrderedDict()
                # print(attr,index)
                attr[key]["score"]=float(pred_logits[i][index])
                attr[key]['name']=label_class_map[key][index_list[i][j]]
                att_num = len(label_class_map[key])
                index_begin+=att_num
                j+=1
            json_init = {
            ...
            testjosn_path = f"/mnt/home/person_att/testjson/{imgs_paths[i]}.json"
            with open(testjosn_path, "w", encoding='utf-8') as f:
    ...
    def confMax(index_list,gt_labels,label_class_map):
        ...
        for i,key in enumerate(label_class_map):
            ...
            testpng_path = f"/mnt/home/person_att/testquality/{key}.png"
